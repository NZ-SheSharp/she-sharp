...
| `DNS_ACCOUNT_MIGRATION.md` | `shesharp.org.nz` is served from a Cloudflare zone in a personal account, beside seven unrelated personal domains — so one person controls where the site points and every piece of email authentication. What has to happen, why Cloudflare makes it a delete-and-recreate rather than a transfer, the outage window that creates, and what must be captured before anyone starts | current — **decided, not done**, and blocked: there is no working registrar login today, and the DNS export the move depends on has not been taken |
